...
function getSearch()
{
	...
	$fields = array(0 => array("field" => "ID", "name" => "ID", "selected" => false), 
		1 => array("field" => $field, "name" => $name, "selected" => true));
	$trash = null;
...
function getSituation($situation, $id, $author)
{
	global $Load;
	...
	if ($edit) {
		if ($situation === "Pending") {
			...
			{ parent = $(this).parent(); link = $(this).detach(); parent.html('". htmlentities(__("Processing")) ."'); $.get('". path("$application/cpanel/activate/$id/$author/") ."', function(data) 
