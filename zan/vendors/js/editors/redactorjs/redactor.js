...
(function($){
	...
	var Redactor = function(element, options)
	{
		...
		this.opts = $.extend({
			...
			modal_code: String() + 
				'<form id="redactorInsertCodeForm">' +
					'<label>' + RLANG.code_content + '</label>' +
					'<textarea id="redactor_insert_code_area" style="width: 99%; height: 160px;"></textarea>' +
				'</form>' +
				'<div id="redactor_modal_footer">' +
					'<span class="redactor_btns_box">' +
						'<input type="button" id="redactor_insert_code_btn" value="' + RLANG.insert + '" />' +
						'<a href="javascript:void(null);" id="redactor_btn_modal_close">' + RLANG.cancel + '</a>' +
					'</span>' +
				'</div>',
	...
	Redactor.prototype = {
		...
		formattingRemoveBr: function(html)
		{
			html = html.replace(/<br ?\/?>/gi, '\n');
		...
		formatting: function(html)
		{
			...
			$.each(html.split($.browser.msie ? '<pre contenteditable="false">' : '<pre>'), $.proxy(function(i, v) {
				...
				// remove formatting
				text = this.formattingRemove(text);

				// empty tags
				text = this.formattingEmptyTags(text);
							
				// add formatting before
				text = this.formattingAddBefore(text);
				
				// add formatting after
				text = this.formattingAddAfter(text);

				// indenting
				text = this.formattingIndenting(text);
		...
		buildButton: function(key, s)
		{
			...
			if (typeof s.exec !== 'undefined')
			{
				button.click($.proxy(function() { this.execCommand(s.exec, key); }, this));
			}
			else if(typeof s.insert !== 'undefined') {
				button.click($.proxy(function() { this.insertHtml(s.insert); }, this));
			}
			else if (s.func !== 'show')
			{
		...
		showCode: function()
		{
			if ($.browser.msie)
			{
				...
				this.modalInit(RLANG.code, 'code', 600, $.proxy(function()
				{
